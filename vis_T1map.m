...
saveas(gcf,strcat('T1wI_WholeVolume_jet.pdf'));
%}
%Flip color jet
figure(2); hold on;
subplot_tight(1,5,1,margins), imshow(T1_est_pre(:,:,1)), title('Pre')
...
saveas(gcf,strcat('T1wI_WholeVolume_flipjet.pdf'));
%{
%Show all slices
...
colorbar; colormap(flip_cb); set(gca,'clim',cb);
subplot_tight(3,5,10), imshow(T1_est_24h(:,:,3)); title('Z2 24hour');
colorbar; colormap(flip_cb); set(gca,'clim',cb);
...
colorbar; colormap(flip_cb); set(gca,'clim',cb);
saveas(gcf,strcat('T1wI_WholeVolume_flipjet_allfig.pdf'));
...
colorbar; colormap jet; set(gca,'clim',cb_rel);
saveas(gcf,strcat('T1wI_WholeVolume_rel_jet.pdf'));
%}
...
save('T1_dynamic.mat','T1_pre','T1_15m','T1_1h','T1_5h','T1_24h');
%}
%% Zoom in
%{
figure(2); hold on;

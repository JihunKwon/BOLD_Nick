%% Parameters
animal_name = 'M4';
time_name = 'PostRTc'; %'PreRT'or 'PostRT'
...
for i=1:size(values_scaled_rel,3)
    figure;
    plot(values_scaled_rel(:,:,i),'-o','LineWidth',2);
...
%% Save parameters
cd(dir_t1map)
save('TOLD.mat','values_scaled','values_scaled_rel','values','values_rel');
...
for i=1:size(values_scaled_rel,3)
    figure;
    plot(values_rel_b(:,1,i),'-s','LineWidth',2); hold on;
    plot(values_rel_b(:,2,i),'-o','LineWidth',2); hold on;
